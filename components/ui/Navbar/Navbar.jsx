...
const Navbar = () => {
    ...
    const navigation = [
        ...
        // { name: "Instruções", href: "/#testimonials" },
        { name: "Como usar", href: "/#faqs" },
    ]
    ...
    return (
        <header className=' fixed top-0 left-0 right-0 y-0 z-30 '>
            <div className="custom-screen bg-gray-900 backdrop-blur-md bg-opacity-90  md:hidden">
                <NavHeader menuBtnEl={menuBtnEl} state={state} onClick={() => setState(!state)} />
            </div>
            <nav className={`pb-5 md:text-sm md:static md:block bg-gray-900 bg-opacity-90 backdrop-blur-md   ${state ? " absolute z-20 top-0 inset-x-0 rounded-b-2xl shadow-xl md:bg-gray-900 md:backdrop-blur-md" : "hidden"}`}>
